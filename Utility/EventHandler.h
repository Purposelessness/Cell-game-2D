...
class EventHandler {
 public:
  ...
  void add(T* obj, void (T::*func)(Args...)) {
    _containers.emplace_back(
        std::make_unique<event_handler::MethodContainer<T, Args...>>(obj,
                                                                     func));
  }

  template <typename T>
  void add(T* obj, void (T::*func)(const Args&...)) {
    _const_containers.emplace_back(
        std::make_unique<event_handler::MethodContainer<T, const Args&...>>(
            obj, func));
  }
  ...
  void remove(T* obj, void (T::*func)(Args...)) {
    for (size_t i = 0; i < _containers.size(); ++i) {
      auto container =
          dynamic_cast<event_handler::MethodContainer<T, Args...>*>(
              _containers[i].get());
  ...
  void remove(T* obj, void (T::*func)(const Args&...)) {
    for (size_t i = 0; i < _const_containers.size(); ++i) {
      auto container =
          dynamic_cast<event_handler::MethodContainer<T, const Args&...>*>(
              _const_containers[i].get());
      if (container == nullptr) {
        continue;
      }
      if (container->getObject() == obj && container->getFunction() == func) {
 ...
 private:
  ...
  std::vector<std::unique_ptr<event_handler::IMethodContainer>>
      _const_containers;
};
