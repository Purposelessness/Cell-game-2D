...
void Logger::operator()(const LogMessage& message) {
  if ((!_extended_filtering && message.level < _filter_level) ||
      (_extended_filtering && !_levels_enabled.at(message.level))) {
    return;
  }
...
void Logger::setFilterLevel(Logger::Level level) { _filter_level = level; }

std::vector<std::string>& Logger::messages() { return _messages; }

SubLogger& Logger::logger(Logger::Level level) { return *_loggers.at(level); }

Logger::Logger() {
  for (int i = 0; i < kLevelCount; ++i) {
    _loggers.at(i) = std::make_unique<SubLogger>(kLevels.at(i));
  }
}

void Logger::enableExtendedFiltering() { _extended_filtering = true; }

void Logger::disableExtendedFiltering() { _extended_filtering = false; }
...
void Logger::disableFilterLevel(Level level) {
  ...
  _levels_enabled.at(level) = false;
}
