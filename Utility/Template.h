...
template <typename T, typename U>
using is_base_of =
    std::is_base_of<remove_cv_reference<T>, remove_cv_reference<U>>;
...
struct are_same<std::tuple<Ts...>, std::tuple<Us...>>
    : all_true<is_same<Ts, Us>...> {};
...
struct extract_method_arguments<F<T, Args...>> {
  ...
  template <size_t I>
  using get_arg_type =
      typename extract_tuple_type_by_index<I, args_tuple>::value;
};
...
template <template <typename...> class Tuple, typename T,
          template <typename...> class OutTuple, typename... OutArgs>
...
struct unique_ptr_tuple_impl<Tuple<T, Args...>, OutTuple<OutArgs...>> {
  using type = typename unique_ptr_tuple_impl<
      Tuple<Args...>, OutTuple<OutArgs..., std::unique_ptr<T>>>::type;
};
...
struct unique_ptr_tuple<Tuple<Args...>> {
  using type = typename details::unique_ptr_tuple_impl<Tuple<Args...>,
                                                       std::tuple<>>::type;
};
...
struct has_all_type_impl<std::tuple<Args...>, std::tuple<T>>
    : has_type<std::tuple<Args...>, typename remove_smart_pointer<T>::type> {};

template <typename... Args, typename T, typename... Ts>
struct has_all_type_impl<std::tuple<Args...>, std::tuple<T, Ts...>>
    : std::false_type {};

template <typename... Args, typename T, typename... Ts>
  requires(has_type<std::tuple<Args...>,
                    typename remove_smart_pointer<T>::type>::value)
struct has_all_type_impl<std::tuple<Args...>, std::tuple<T, Ts...>>
    : has_all_type_impl<std::tuple<Args...>, std::tuple<Ts...>> {};
}  // namespace details
...
struct has_all_type<std::tuple<Args...>, std::tuple<Ts...>>
    : details::has_all_type_impl<std::tuple<Args...>, std::tuple<Ts...>> {};
