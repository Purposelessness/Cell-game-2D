...
template <typename T>
concept TInjectClient = requires(T t) { &T::inject; };
...
struct shared_ptr_tuple_impl<Tuple<T, Args...>, OutTuple<OutArgs...>> {
  using type = typename shared_ptr_tuple_impl<
      Tuple<Args...>, OutTuple<OutArgs..., std::shared_ptr<T>>>::type;
};
...
struct shared_ptr_tuple<Tuple<Args...>> {
  using type = typename details::shared_ptr_tuple_impl<Tuple<Args...>,
                                                       std::tuple<>>::type;
};
