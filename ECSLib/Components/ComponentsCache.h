...
class CommonCache {
 public:
  explicit CommonCache(int type_id = -1) : type_id(type_id) {}

  virtual ~CommonCache() = default;
  ...
  virtual bool hasComponent(int id) { return false; }
...
class ComponentsCache : public CommonCache {
 public:
  ComponentsCache() : CommonCache(TypeIdentifier<T>::getId()) {}
  ...
  T& getComponent(int entity_id) { return _data[entity_id]; }
  ...
  void removeComponent(int entity_id) override {
    ...
    _keys.erase(std::remove(_keys.begin(),  // NOLINT(bugprone-inaccurate-erase)
                            _keys.end(), entity_id));
  }
